...
Adapted from ai-service/app/agents/research/base.py.
Max iterations default 3 (higher than ai-service) to collect more data per destination.
"""
...
from app.config.settings import llm, llm_with_tools, console, MAX_RESEARCH_ITERATIONS
from app.utils.search_tools import _run_safe_tool_calls, _is_malformed_tool_call
...
class ResearchAgentState(TypedDict):
    ...
    result: str  # accumulated research text
...
def _make_research_node(domain: str, prompt_template: str, log_label: str):
    def agent_node(state: ResearchAgentState) -> dict:
        ...
        if not any(isinstance(m, HumanMessage) for m in existing):
            existing = [HumanMessage(
                content=f"Research {domain} for {ctx.get('destination', '')}. Call web_search now — do NOT output JSON."
            )] + existing
    ...
    def summarize_node(state: ResearchAgentState) -> dict:
        """When max iterations hit, force LLM to summarize all collected data as text."""
        ctx = state["context"]
        system = prompt_template.format(**ctx)
        summary_prompt = HumanMessage(
            content=(
                "You have reached the maximum number of search iterations. "
                "Based on ALL the search results above, write a comprehensive summary "
                "of everything you found. Include all place names, addresses, prices, "
                "opening hours, and other details. Do NOT call any tools — just write the summary now."
            )
        )
        response = llm.invoke(
            [SystemMessage(content=system)] + list(state["messages"]) + [summary_prompt]
        )
        console.print(f"  [green]{log_label}[/green] summarized {len(response.content)} chars.")
    ...
    def router(state: ResearchAgentState) -> str:
        ...
        if hasattr(last, "tool_calls") and last.tool_calls:
            ...
            if rounds >= MAX_RESEARCH_ITERATIONS:
                console.print(f"  [yellow]{log_label}[/yellow] max iterations ({MAX_RESEARCH_ITERATIONS}) reached — routing to summarize.")
                return "summarize"
    ...
    g.add_conditional_edges("agent", router, {"tools": "tools", "fix_tools": "fix_tools", "summarize": "summarize", END: END})
